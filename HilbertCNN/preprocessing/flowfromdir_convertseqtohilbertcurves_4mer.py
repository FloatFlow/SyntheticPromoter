...
kmer_perms = list(''.join(kmer) for kmer in itertools.product(['A', 'C', 'G', 'T'], repeat=4)) #was originally in kmer_onehotcoder, with repeat=mer_size
...
write_folder = 'D:/Projects/iSynPro/iSynPro/HilbertCNN/train_val_npys/test'
def multiprocessing_write_wrapper(i):
    # create and write hilbert curves
    hilbert_curve = hilbert_image_generator(kmer_onehotcoder(data['sequence'][i], 4, hot_dict))
    if data['y'][i] == 0:

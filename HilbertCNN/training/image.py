...
class ImageDataGenerator(object):
    ...
    def flow_np_from_directory(self, directory,
                            target_size, color_mode='rgb',
                            classes=None, class_mode='categorical',
                            batch_size=32, shuffle=True, seed=None,
                            save_to_dir=None, save_prefix='', save_format='jpeg'):
        return NumpyDirectoryIterator(
            directory, self, target_size, color_mode=color_mode,
            classes=classes, class_mode=class_mode,
            dim_ordering=self.dim_ordering,
            batch_size=batch_size, shuffle=shuffle, seed=seed,
            save_to_dir=save_to_dir, save_prefix=save_prefix, save_format=save_format)
...
class NumpyDirectoryIterator(Iterator):
        ...
        for subdir in classes:
            ...
            for fname in sorted(os.listdir(subpath)):
                ...
                if is_valid:
                    ...
                    self.filenames.append(os.path.join(subdir, fname))
                    i += 1
        super(NumpyDirectoryIterator, self).__init__(self.nb_sample, batch_size, shuffle, seed)
